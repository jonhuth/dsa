class MinHeap:
    ...
    def swim(self, k):
        ...
        exchange with parent as long as parent is larger.
    ...
    def sink(self, k):
        ...
        exchange with max child as long as parent is larger.
        time: O(logn) | space: O(1)
        '''
        n = self.size()
        while 2*k <= n:
            j = 2*k
            if j < n and self.heap[j] > self.heap[j+1]:  # j is smaller child idx
                j += 1
