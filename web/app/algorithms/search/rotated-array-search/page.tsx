...
export default function RotatedArraySearchPage() {
	...
	const algorithm = registry.algorithms.get("rotated_search");
	const relatedAlgorithms = algorithm ? getRelatedAlgorithms("rotated_search") : [];
	const prerequisites = algorithm ? getPrerequisites("rotated_search") : [];

	// Fetch source code on mount
	useEffect(() => {
		const fetchSource = async () => {
			try {
				const response = await fetch("/api/algorithms/rotated_search/source");
	...
	const executeAlgorithm = async () => {
		...
		try {
			...
			const response = await fetch("/api/algorithms/rotated_search/execute", {
				method: "POST",
				headers: {
									...
									<div className="p-4 sm:p-6 border border-border rounded-lg order-1 md:order-2">
										...
										{currentStepData?.metadata && (
											<div className="mt-3 sm:mt-4 flex flex-wrap justify-center gap-4 sm:gap-6 text-xs">
												...
												{currentStepData.metadata.sorted_half !== undefined && (
													<div>
														Sorted half:{" "}
														<span className="font-mono">
															{currentStepData.metadata.sorted_half as string}
														</span>
													</div>
												)}
											</div>
