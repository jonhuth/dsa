...
export default function TernarySearchPage() {
	...
	const algorithm = registry.algorithms.get("ternary_search");
	const relatedAlgorithms = algorithm ? getRelatedAlgorithms("ternary_search") : [];
	const prerequisites = algorithm ? getPrerequisites("ternary_search") : [];

	// Fetch source code on mount
	useEffect(() => {
		const fetchSource = async () => {
			try {
				const response = await fetch("/api/algorithms/ternary_search/source");
	...
	const executeAlgorithm = async () => {
		...
		try {
			...
			const response = await fetch("/api/algorithms/ternary_search/execute", {
				method: "POST",
				headers: {
							...
							<div className="space-y-3 sm:space-y-4">
								...
								>
									...
									<div className="p-4 sm:p-6 border border-border rounded-lg order-1 md:order-2">
										...
										{currentStepData?.metadata && (
											<div className="mt-3 sm:mt-4 flex flex-wrap justify-center gap-4 sm:gap-6 text-xs">
												...
												{currentStepData.metadata.lo !== undefined && (
													<div>
														Lo: <span className="font-mono">{currentStepData.metadata.lo}</span>
													</div>
												)}
												{currentStepData.metadata.mid1 !== undefined && (
													<div>
														Mid1: <span className="font-mono">{currentStepData.metadata.mid1}</span>
													</div>
												)}
												{currentStepData.metadata.mid2 !== undefined && (
													<div>
														Mid2: <span className="font-mono">{currentStepData.metadata.mid2}</span>
													</div>
												)}
												{currentStepData.metadata.hi !== undefined && (
													<div>
														Hi: <span className="font-mono">{currentStepData.metadata.hi}</span>
													</div>
												)}
												{currentStepData.metadata.surviving_third !== undefined && (
													<div>
														Kept third:{" "}
														<span className="font-mono">
															{currentStepData.metadata.surviving_third as string}
														</span>
													</div>
												)}
											</div>
								...
								<div className="flex flex-wrap items-center justify-center gap-6 text-xs">
									<div className="flex items-center gap-2">
										...
										<span>Split Points (mid1, mid2)</span>
									</div>
									<div className="flex items-center gap-2">
										<div className="w-4 h-4 bg-yellow-500 rounded" />
										<span>Surviving Third / Window</span>
									</div>
									<div className="flex items-center gap-2">
