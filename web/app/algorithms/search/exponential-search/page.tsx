...
export default function ExponentialSearchPage() {
	...
	const [inputArray, setInputArray] = useState("1, 3, 5, 7, 9, 11, 13, 17, 21, 25");
	const [target, setTarget] = useState("17");
	const [sourceCode, setSourceCode] = useState<string>("");
	const [showCode, setShowCode] = useState(true);

	// Get algorithm metadata
	const algorithm = registry.algorithms.get("exponential_search");
	const relatedAlgorithms = algorithm ? getRelatedAlgorithms("exponential_search") : [];
	const prerequisites = algorithm ? getPrerequisites("exponential_search") : [];

	// Fetch source code on mount
	useEffect(() => {
		const fetchSource = async () => {
			try {
				const response = await fetch("/api/algorithms/exponential_search/source");
	...
	const executeAlgorithm = async () => {
		...
		try {
			...
			const response = await fetch("/api/algorithms/exponential_search/execute", {
				method: "POST",
				headers: {
									...
									<div className="p-4 sm:p-6 border border-border rounded-lg order-1 md:order-2">
										...
										{currentStepData?.metadata && (
											<div className="mt-3 sm:mt-4 flex flex-wrap justify-center gap-4 sm:gap-6 text-xs">
												{currentStepData.metadata.phase !== undefined && (
													<div>
														Phase:{" "}
														<span className="font-mono capitalize">
															{currentStepData.metadata.phase as string}
														</span>
													</div>
												)}
												...
												{currentStepData.metadata.bound !== undefined && (
													<div>
														Bound:{" "}
														<span className="font-mono">
															{currentStepData.metadata.bound as number}
														</span>
													</div>
												)}
