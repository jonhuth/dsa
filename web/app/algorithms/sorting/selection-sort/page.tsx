...
export default function SelectionSortPage() {
	...
	const [isLearned, toggleLearned] = useLearnedState("selection_sort");

	const { randomize } = useRandomArray();
	const randomizeArray = () => setInputArray(randomize().join(", "));

	const algorithm = registry.algorithms.get("selection_sort");
	const relatedAlgorithms = algorithm ? getRelatedAlgorithms("selection_sort") : [];
	const prerequisites = algorithm ? getPrerequisites("selection_sort") : [];

	useEffect(() => {
		const fetchSource = async () => {
			try {
				const response = await fetch("/api/algorithms/selection_sort/source");
									...
									<div className="p-4 sm:p-6 border border-border rounded-lg order-1 md:order-2">
										...
										{currentStepData?.metadata && (
											<div className="mt-3 sm:mt-4 flex justify-center gap-4 sm:gap-6 text-xs flex-wrap">
												...
												{currentStepData.metadata.current_min !== undefined && (
													<div>
														Current Min:{" "}
														<span className="font-mono">
															{currentStepData.metadata.current_min as number}
														</span>
													</div>
												)}
											</div>
