...
- [x] **Live code execution viewer** (see highlighted source as viz runs)
- [x] **Algorithm metadata registry system** (`web/lib/`)
  - Types, tags, algorithms metadata with rich learning content
  - Categories metadata with icons and descriptions
  - Relationships system for cross-linking algorithms
  - Central registry for accessing all content
- [x] **Learning components** for rich documentation:
  - ComplexityChart - Visual Big-O comparison
  - EdgeCases - Edge cases and pitfalls display
  - KeyInsights - "Aha" moments highlights
  - RelatedLinks - Related algorithms sidebar
  - Prerequisites - Prerequisite algorithms
  - WhenToUse - Real-world use cases
  - InterviewTips - Interview-specific advice
- [x] **Refactored pages to use registry**:
  - Landing page now pulls from registry
  - Algorithms index shows all active categories
  - Category pages dynamically generated from registry

### 🚧 In Progress / TODO

See `docs/todo.md` for full roadmap. High-priority items:

- [ ] **Integrate learning components into exhibit pages** - Components created but not yet used in algorithm pages
- [ ] More sorting algorithms (Quick Sort, Merge Sort, etc.) - Many already exist, need to verify all work
- [ ] Graph visualizer and algorithms (BFS, DFS) - Visualizers and algorithms exist
- [ ] Tree visualizer and algorithms - Visualizers and algorithms exist
- [ ] AI chat integration (Anthropic Sonnet)
- [ ] Global search dialog (Cmd+K) - Basic search exists, needs dialog component
